...
/** 
 * <b>Ενημέρωση Υποδικτύων Μονάδας</b>
 * 
 * 
 * Η συνάρτηση αυτή ενημερώνει Υποδικτύα Μονάδας σύμφωνα με τις παραμέτρους που έγινε η κλήση
 *
 *
 * Η κλήση μπορεί να γίνει μέσω της παρακάτω διεύθυνσης με τη μέθοδο PUT :
 * <br> https://mm.sch.gr/api/unit_network_subnets <br><br>
 *
 *
 ...
 * <ul>
 *  <li><b>unit_network_subnet_id</b><br>GetUnitNetworkSubnets με search_type=EXACT & unit_network_subnet_id="value" </li>
 ...
 * <code>
 * curl -X PUT https://mm.sch.gr/api/unit_network_subnets \
 ...
 *    http.open("PUT", "https://mm.sch.gr/api/unit_network_subnets");
 ...
 * <br>H Μάσκα του Υποδικτύου της Μονάδας είναι υποχρεωτικό 
 * 
...
function PutUnitNetworkSubnets(
    $unit_network_subnet_id, $subnet_name, $subnet_ip, $subnet_default_router, $mask,
...
//======================================================================================================================
        ...
        $sqlWhere = " WHERE ". $filters["unit_network_subnet_id"];

        unset($filters["unit_network_subnet_id"]);

        $sql = "UPDATE unit_network_subnets SET " . implode(", ", $filters) .$sqlWhere;
        //echo "<br><br>".$sql."<br><br>";
        $array_sql[] = trim( preg_replace('/\s\s+/', ' ', $sql));
