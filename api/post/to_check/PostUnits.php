...
//==============================================================================
        ...
        $stmt = $db->query( "SELECT category_id FROM categories WHERE ".$sqlWhere );
...
//==============================================================================
        ...
        $stmt = $db->query( "SELECT source_id FROM sources WHERE ".$sqlWhere );
        $rows = $stmt->fetch(PDO::FETCH_ASSOC);
...
//==============================================================================
        ...
        $stmt = $db->query( "SELECT unit_type_id FROM unit_types WHERE ".$sqlWhere );
        $rows = $stmt->fetch(PDO::FETCH_ASSOC);
...
//==============================================================================
        ...
        if  ( ( ($category == 3) && ($source == 1) && (in_array($unit_type, array(12, 13))) ) )
            $search_filter[] = "name = '". mysql_escape_string( $name ) ."'";
        else
            $search_filter[] = "registry_no = '". mysql_escape_string( $registry_no ) ."'";
                
        $search_filter[] = "source_id = ".$source;
        
        $sql = "SELECT mm_id FROM units WHERE ".implode(" AND ", $search_filter);
        ...
        if ( $db_row["mm_id"] )
        {
            throw new Exception(ExceptionMessages::DuplicatedUnitValue." : ".$db_row["mm_id"], ExceptionCodes::DuplicatedUnitValue);
        }
        else
        {
...
//==============================================================================
            ...
            if ($sqlWhere)
            {
                $stmt = $db->query( "SELECT state_id FROM states WHERE ".$sqlWhere );
                $rows = $stmt->fetch(PDO::FETCH_ASSOC);
...
//==============================================================================
            ...
            $sqlUpdate = "INSERT INTO units SET ";
