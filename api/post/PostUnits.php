...
function unitsSetAssociation(&$unit, $param, $repo, $field, $exceptionType, $required = true) {
    ...
    if ( $param === _MISSED_ ) {
        if(!$required) { return; }
        throw new Exception($missingParam, $missingParam);
    } else if ( Validator::IsNull($param) ) {
        if(!$required) { return; }
        throw new Exception($missingValue, $missingValue);
        ...
        throw new Exception($invalidType." : ".$param, $invalidType);

    if ( !isset($retriedObject) )
        throw new Exception($invalidValue." : ".$param, $invalidValue);
    else
    {
