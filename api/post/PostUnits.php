 ...
 * **Καταχώρηση Μονάδας**
 *
 * Η συνάρτηση καταχωρεί Μονάδα σύμφωνα με τις παραμέτρους που έγινε η κλήση.
 * <br>Η κλήση μπορεί να γίνει μέσω της παρακάτω διεύθυνσης με τη μέθοδο **POST** και route_api_name = **units** :
 ...
 * <br>integer : <b>mm_id</b> : Ο Κωδικός ID της Μονάδας που δημιουργήθηκε
 ...
 *
 * @throws NoErrorsImportUnit {@see ExceptionMessages::NoErrorsImportUnit}
 * <br>{@see ExceptionCodes::NoErrorsImportUnit}
 * 
 * 
 */

function PostUnits( $registry_no, $source, $name, $special_name, $state, $region_edu_admin, 
    ...
    //insert to db==============================================================
       ...
       //SendMail('mm@sch.gr', 'Δημιουργία Μονάδας', 'Δημιουργία Μονάδας με κωδικό ΜΜ : '.$result["mm_id"]);
                 
//result_messages===============================================================      
        $result["mm_id"] = $unit->getMmId();
        $result["status"] = ExceptionCodes::NoErrors;
        $result["message"] = "[".$result["method"]."][".$result["function"]."]:".ExceptionMessages::NoErrorsImportUnit;
    } catch (Exception $e) {
