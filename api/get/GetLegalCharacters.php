...
/**
* **ΛΕΞΙΚΟ : Νομικοί Χαρακτήρες Μονάδων**
 * 
 * Η συνάρτηση αυτή επιστρέφει Νομικούς Χαρακτήρες Μονάδων σύμφωνα με τις παραμέτρους που έγινε η κλήση.
 * <br>Η κλήση μπορεί να γίνει μέσω της παρακάτω διεύθυνσης με τη μέθοδο **GET** και route_api_name = **legal_characters** :
 ...
 *  "status": 200,
 *  "message": "[GET][legal_characters]:success"
 * }
 * </code>
 *
 * ***Πίνακας Δεδομένων***
 ...
 * 
 * 
 * @param mixed $legal_character Νομικός Χαρακτήρας Μονάδων 
 * <br>
 * <br>Το Όνομα ή ο Κωδικός ID του Νομικού Χαρακτήρα Μονάδων 
 * <br>Η τιμή της παραμέτρου μπορεί να είναι : mixed{integer|string|array[integer|string]}
 * <ul>
 *    <li>integer
 *       <br>Αριθμητική : Η αναζήτηση γίνεται με τον Κωδικό ID του Νομικού Χαρακτήρα Μονάδων
 *       <br>Η αναζήτηση στον Κωδικό γίνεται με τον Τύπο {@see SearchEnumTypes::Exact}
 *    </li>
 *    <li>string
 *       <br>Αλφαριθμητική : Η αναζήτηση γίνεται με το Όνομα του Νομικού Χαρακτήρα Μονάδων
 ...
 *  <ul>
 *    <li>integer : <b>legal_character_id</b> : Ο Κωδικός ID του Νομικού Χαρακτήρα</li>
...
function GetLegalCharacters( $legal_character, $category,
...
//$legal_character==============================================================
        if (Validator::Exists('legal_character', $params)){
            CRUDUtils::setFilter($qb, $legal_character, "lc", "legalCharacterId", "name", "id,value", ExceptionMessages::InvalidLegalCharacterType, ExceptionCodes::InvalidLegalCharacterType);
        }
...
//execution=====================================================================
        $qb->select('lc');
        $qb->from('LegalCharacters','lc');
        $qb->leftjoin('lc.category','c');
...
//data results==================================================================       
        ...
        foreach ($results as $row)
        {
            ...
            $result["data"][] = array(
                                        "legal_character_id" => $row->getLegalCharacterId(),
                                        "legal_character"    => $row->getName(),
