...
$units_array = unserialize( file_get_contents('data/units_data.txt') );

function loadData($api_url)
{
    global $Options;
    
    $server = $Options["ServerURL"].$api_url;
...
$msg = "Συγχρονισμός Σχολικών Μονάδες με Survey";
...
{
    $region_edu_admins[$item->region_edu_admin_id] = $item->name;
}
...
foreach ($data->data as $item)
{
    $edu_admins[$item->edu_admin_id] = $item->name;
}
...
foreach ($data->data as $item)
{
    $transfer_areas[$item->transfer_area_id] = $item->name;
}
...
foreach ($data->data as $item)
{
    $municipalities[$item->municipality_id] = $item->name;
}
...
foreach ($data->data as $item)
{
    $prefectures[$item->prefecture_id] = $item->name;
}


$msg = "Έναρξη Συγχρονισμού : ".date("d/m/Y H:i:s");
echo $msg.$br.$br; $logMessage[] = $msg."\n";
...
foreach ($units_array as $result)
{
    //if ($totalRowsCounter == 100) break;
        ...
        (trim($unit["Perifereia"]) <> "ΣΙΒΙΤΑΝΙΔΕΙΟΣ"))
    {
        ...
        if ($unit["Perifereia"])
        {
            if (!in_array($unit["Perifereia"], $region_edu_admins))
            {
                $sql = "INSERT INTO region_edu_admins SET name='".mysql_escape_string($unit["Perifereia"])."'";
                $db->query($sql);
                $region_edu_admins[ $db->lastInsertId() ] = $unit["Perifereia"];
            }
        }
        $region_edu_admin_id = array_search($unit["Perifereia"], $region_edu_admins);


        $unit["Diefthinsi"] = trim($unit["Diefthinsi"]);
        if ($unit["Diefthinsi"])
        {
            if (!in_array($unit["Diefthinsi"], $edu_admins))
            {
                ...
                $db->query($sql);
                $edu_admins[ $db->lastInsertId() ] = $unit["Diefthinsi"];
            }
        }
        $edu_admin_id = array_search($unit["Diefthinsi"], $edu_admins);


        $unit["PostingTransferArea"] = trim($unit["PostingTransferArea"]);
        if ($unit["PostingTransferArea"])
        {
            if (!in_array($unit["PostingTransferArea"], $transfer_areas))
            {
                ...
                $db->query($sql);
                $transfer_areas[ $db->lastInsertId() ] = $unit["PostingTransferArea"];
            }
//            else if ($edu_admin_id)
//            {
//                $transfer_area_id = array_search($unit["PostingTransferArea"], $transfer_areas);
//
...
//            }
        ...
        if ($unit["Prefecture"])
        {
            if (!in_array($unit["Prefecture"], $prefectures))
            {
                $sql = "INSERT INTO prefectures SET "
                        . "name='".mysql_escape_string($unit["Prefecture"])."'";
                $db->query($sql);
                $prefectures[ $db->lastInsertId() ] = $unit["Prefecture"];
            } 
        }
        $prefecture_id = array_search($unit["Prefecture"], $prefectures);
        ...
        {
            if (!in_array($unit["Municipality"], $municipalities))
            {
                ...
                $db->query($sql);
                $municipalities[ $db->lastInsertId() ] = $unit["Municipality"];
            }                        
//            else if ($transfer_area_id && $prefecture_id)
//            {
//                $municipality_id = array_search($unit["Municipality"], $municipalities);
//
