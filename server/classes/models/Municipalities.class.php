...
class Municipalities extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	...
	const SQL_INSERT='INSERT INTO `municipalities` (`municipality_id`,`name`,`prefecture_id`,`transfer_area_id`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `municipalities` (`name`,`prefecture_id`,`transfer_area_id`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `municipalities` SET `municipality_id`=?,`name`=?,`prefecture_id`=?,`transfer_area_id`=? WHERE `municipality_id`=?';
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getMunicipalityId()) {
			...
			$stmt->bindValue(2,$this->getPrefectureId());
	...
	/**
	 * Fetch Units's which this Municipalities references.
	 * `municipalities`.`municipality_id` -> `units`.`municipality_id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return Units[]
	...
	public function fetchUnitsCollection(PDO $db, $sort=null) {
		$filter=array(Units::FIELD_MUNICIPALITY_ID=>$this->getMunicipalityId());
