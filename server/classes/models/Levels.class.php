 ...
 * @package entity
 */
class Levels extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='Levels';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='levels';
	const SQL_INSERT='INSERT INTO `levels` (`level_id`,`name`,`groups_count`,`students_count`,`mm_id`) VALUES (?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `levels` (`name`,`groups_count`,`students_count`,`mm_id`) VALUES (?,?,?,?)';
	const SQL_UPDATE='UPDATE `levels` SET `level_id`=?,`name`=?,`groups_count`=?,`students_count`=?,`mm_id`=? WHERE `level_id`=?';
	const SQL_SELECT_PK='SELECT * FROM `levels` WHERE `level_id`=?';
	const SQL_DELETE_PK='DELETE FROM `levels` WHERE `level_id`=?';
	const FIELD_LEVEL_ID=-1320959659;
	const FIELD_NAME=-1661932630;
	const FIELD_GROUPS_COUNT=-1406549757;
	const FIELD_STUDENTS_COUNT=1513537031;
	const FIELD_MM_ID=19116763;
	private static $PRIMARY_KEYS=array(self::FIELD_LEVEL_ID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_LEVEL_ID);
	...
	private static $FIELD_TYPES=array(
		self::FIELD_LEVEL_ID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of Levels instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param Levels $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return Levels[]
	 */
	public static function findByExample(PDO $db,Levels $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of Levels instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of Levels instances
	 *
	...
	/**
	 * returns the result as an array of Levels instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return Levels[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of Levels instances.
	 *
	 ...
	 * @return Levels
	 */
	public static function findById(PDO $db,$levelId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$levelId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getLevelId());
		$stmt->bindValue(2,$this->getName());
		$stmt->bindValue(3,$this->getGroupsCount());
		$stmt->bindValue(4,$this->getStudentsCount());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getLevelId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getName());
			$stmt->bindValue(2,$this->getGroupsCount());
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(6,$this->getLevelId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch Groups's which this Levels references.
	 * `levels`.`level_id` -> `groups`.`level_id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return Groups[]
	...
	public function fetchGroupsCollection(PDO $db, $sort=null) {
		$filter=array(Groups::FIELD_LEVEL_ID=>$this->getLevelId());
		return Groups::findByFilter($db, $filter, true, $sort);
	}

	/**
	 * Fetch Units which references this Levels. Will return null in case reference is invalid.
	 * `levels`.`mm_id` -> `units`.`mm_id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return Units
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'Levels');
	}

	/**
	 * get single Levels instance from a DOMElement
	 *
	...
	/**
	 * get all instances of Levels from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('Levels') as $node) {
